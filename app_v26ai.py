...
.rate-cards { display: flex; gap: 10px; margin-bottom: 1rem; flex-wrap: wrap; }
.rate-card { flex: 1; min-width: 120px; background: #f8fdf9; border-radius: 8px; padding: 10px 14px; border-left: 3px solid #1D9E75; }
.rate-name { font-size: 12px; color: #666; margin-bottom: 4px; }
.rate-val { font-size: 22px; font-weight: 600; color: #0F6E56; }
...
.grade-tab { padding: 5px 12px; border-radius: 99px; font-size: 12px; font-weight: 500; cursor: pointer; border: 1px solid #ccc; color: #666; background: #f5f5f5; }
.grade-tab.active { background: #1D9E75; color: white; border-color: #1D9E75; }
...
def save_history(history):
    ...
    token   = st.secrets.get("github_token", "")
    if not gist_id:
        st.error("❌ gist_id not set in Streamlit secrets.")
        return False
    if not token:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            st.error("❌ GitHub token expired or invalid.")
            return False
        elif r.status_code == 404:
            st.error("❌ Gist not found.")
            return False
        else:
            st.error(f"❌ Could not save. Status: {r.status_code}")
            return False
    except Exception as e:
...
def build_reply(lines, total, is_timber=True, extra_note=""):
    """
    is_timber=True  → Thickness/Width AND Length tolerances
    is_timber=False → Thickness/Width tolerance only (plywood)
    """
    out = list(lines)
    out.append(f"\nTotal : S${total:,.2f}")
    out.append("\nTolerances:")
...
# ============================================================
# SESSION STATE — all keys initialised once at startup
# ============================================================
_defaults = {
    "order_items":  [],
    "odd_items":    [],
    ...
    "cust_name":    "",
    "cust_mobile":  "",
    # Persisted generated quote results so Save button works after rerun
    "q_ready":      False,  # Tab1 quote ready flag
    "q_reply":      "",
    "q_total":      0.0,
    "q_cost":       0.0,
    "q_nitem":      0,
    "q_log":        [],
    "odd_ready":    False,  # Tab2 odd size ready flag
    "odd_reply":    "",
    "odd_total":    0.0,
    "odd_cost":     0.0,
    "odd_nitem":    0,
    "odd_log":      [],
    "ply_ready":    False,  # Tab3 plywood ready flag
    "ply_reply":    "",
    "ply_total":    0.0,
    "ply_cost":     0.0,
    "ply_nitem":    0,
    "ply_log":      [],
    "cut_ready":    False,  # Tab3 cut-to-size ready flag
    "cut_reply":    "",
    "cut_total":    0.0,
    "cut_cost":     0.0,
}
...
with tab_quote:
    ...
    if st.session_state.order_items:
        ...
        # Build and store quote when Generate is clicked
        if gen_quote:
            ...
            # Persist everything so Save button survives rerun
            st.session_state.q_ready  = True
            st.session_state.q_reply  = reply_text
            st.session_state.q_total  = grand_total
            st.session_state.q_cost   = cost_total
            st.session_state.q_nitem  = len(customer_reply)
            st.session_state.q_log    = log_items

        # Show generated quote if ready (persists across reruns)
        if st.session_state.q_ready:
            grand_total = st.session_state.q_total
            cost_total  = st.session_state.q_cost
...
with tab_odd:
    ...
    with os2:
        default_rate = species_rate[st.session_state.odd_sp]
        odd_rate = st.number_input("Rate (S$/ton) — adjust if needed", min_value=0, value=default_rate, step=50, key="odd_rate")
        ...
        if st.button("+ Add to Odd Size List", type="primary", use_container_width=True):
            ...
            if cthk and cwid and clen and qthk and qwid and qlen:
                ...
                st.session_state.odd_items.append({
                    "species": st.session_state.odd_sp, "cust_size": cust_size,
                    "quote_size": quote_size, "price": price,
                    "qty": st.session_state.odd_qty, "line_total": line_total,
                    "rate": odd_rate, "pcs_per_ton": pcs_per_ton, "pcs_floor": pcs_floor,
    ...
    if st.session_state.odd_items:
        ...
        for i, item in enumerate(st.session_state.odd_items):
            ...
            with oc:
                if st.button("✏️", key=f"eo_{i}"):
                    st.session_state.odd_items.pop(i)
                    st.session_state.odd_ready = False
                    st.rerun()
            with od:
        ...
        if gen_odd:
            ...
            st.session_state.odd_ready = True
            st.session_state.odd_reply = reply_text
            st.session_state.odd_total = odd_total
            st.session_state.odd_cost  = odd_cost
            st.session_state.odd_nitem = len(odd_reply)
            st.session_state.odd_log   = odd_log

        if st.session_state.odd_ready:
            odd_total = st.session_state.odd_total
            odd_cost  = st.session_state.odd_cost
            render_staff_log(st.session_state.odd_log, odd_total, odd_cost)
            st.divider()
            st.subheader("Customer Reply (edit before sending)")
...
with tab_ply:
    ...
    with ply_sub1:
        ...
        with pg1:
            p_grade = st.selectbox("Grade", PLY_GRADES,
                index=PLY_GRADES.index(st.session_state.ply_cur_grade), key="p_gr_sel")
        ...
        with st.form("ply_add_form", clear_on_submit=True):
            ...
            with fa1:
                ...
                p_sell_f = st.number_input("Selling Price (S$/sheet)",
                    min_value=0.0, value=float(p_sell_def), step=0.5, format="%.2f", key=p_sell_key)
            with fa2:
        ...
        if p_sell_def == 0.0:
            st.warning(f"⚠️ Selling price is S$0.00 — check price table.")
        ...
        if st.session_state.ply_items:
            ...
            for i, item in enumerate(st.session_state.ply_items):
                ...
                with col_b:
                    st.markdown(
                        f"S\\${item['sell']}/sheet &nbsp;×&nbsp; {item['actual_qty']} sheets &nbsp;=&nbsp; **S\\${item['line_total']:,.2f}**",
                        unsafe_allow_html=True)
                with col_c:
            ...
            st.divider()
            ply_grand = sum(x["line_total"] for x in st.session_state.ply_items)
            ply_cost_total = sum(x["cost"] * x["actual_qty"] for x in st.session_state.ply_items)
            ply_grand = round(ply_grand, 2); ply_cost_total = round(ply_cost_total, 2)
            ...
            if gen_ply:
                ...
                for item in st.session_state.ply_items:
                    ...
                    ply_log.append({
                        ...
                        "rows": {
                            "Cost (YC)":  f"S${item['cost']}/sheet",
                            "Selling":    f"S${item['sell']}/sheet",
                ...
                st.session_state.ply_ready = True
                st.session_state.ply_reply = reply_txt
                st.session_state.ply_total = ply_grand
                st.session_state.ply_cost  = ply_cost_total
                st.session_state.ply_nitem = len(ply_reply)
                st.session_state.ply_log   = ply_log

            if st.session_state.ply_ready:
                render_staff_log(st.session_state.ply_log,
                                 st.session_state.ply_total, st.session_state.ply_cost)
                st.divider()
                st.subheader("Customer Reply (edit before sending)")
                ...
                with pl2:
                    if st.button("💾 Save to History", type="primary", key="save_ply", use_container_width=True):
                        ok = save_quote(
                            st.session_state.cust_name, st.session_state.cust_mobile,
                            st.session_state.ply_total, st.session_state.ply_nitem,
                            ply_edited, st.session_state.ply_cost)
                        if ok: st.success("✅ Saved!")
                        else:  st.error("❌ Could not save.")
        else:
    ...
    with ply_sub2:
        ...
        with st.form("cut_form", clear_on_submit=False):
            c_sell = st.number_input("Selling Price per full sheet (S$)",
                min_value=0.0, value=float(c_sell_def), step=0.5, format="%.2f", key=c_sell_key)
        ...
        if calc_cut and c_w and c_l:
            ...
            st.session_state.cut_ready = True
            st.session_state.cut_reply = cut_reply_text
            st.session_state.cut_total = total
            st.session_state.cut_cost  = round(cost_pc * c_qty, 2)
    ...
    with ply_sub3:
        ...
        st.caption("Nominal vs actual thickness by supplier — quick reference when customers query thickness.")
        tol_rows = [
...
with tab_sup:
    ...
    <div class="sup-header">
      ...
      <div>
        <div class="sup-name">Ying Chuan Timber Co Pte Ltd</div>
        <div class="sup-sub">Supplier 1 &nbsp;·&nbsp; Plony Industries &nbsp;·&nbsp; Updated May 2026</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with sup1:
        ...
        if grade_sel in PLY_COST:
            ...
            for thk, cost in sorted(PLY_COST[grade_sel].items()):
                ...
                rows.append({
                    "Thickness":         f"{thk}mm" + (f" ({note})" if note else ""),
    ...
    with sup2:
        ...
        for grade in PLY_GRADES:
            ...
            sells      = list(PLY_SELL[grade].values())
            costs      = [PLY_COST.get(grade, {}).get(t, 0) for t in PLY_SELL[grade].keys()]
...
def parse_smart_text(text):
    ...
    for line in lines:
        ...
        if sp: cur_sp = sp
        dim = re.search(r"(\d+\.?\d*)\s*[xX×/\\]\s*(\d+\.?\d*)", line)
        if dim:
        ...
        if len(parts) >= 5:
            ...
            if sp2:
                try:
                    results.append({
                        "species": sp2,
                        "thk_mm": float(parts[1]), "wid_mm": float(parts[2]),
                        "len_m": float(parts[3]), "qty": int(float(parts[4])),
                        "source": "shorthand"
                    })
                except: pass
    return results
...
with tab_ai:
    ...
    with ai_t1:
        ...
        • <code>坡楼 35×160 &nbsp; 4000=2条 &nbsp; 5200=1条</code><br>
        • <code>kapur 35x80 2.4m 50pcs</code><br>
        • Mixed Chinese/English, multiple sizes in one paste
